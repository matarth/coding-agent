...
- **MCP servers** (`.mcp.json`, `.claude/tools/`): `asana` (hosted,
  OAuth) and `db-readonly` (local, read-only Postgres access) — see below.
- **Skills** (`.claude/skills/`): [`code-review`](.claude/skills/code-review/SKILL.md),
  [`bug-fix`](.claude/skills/bug-fix/SKILL.md), and
  [`docblock-audit`](.claude/skills/docblock-audit/SKILL.md) —
  each a self-contained workflow invoked for a specific kind of task.
- **Subagents** (`.claude/agents/`): [`investigator`](.claude/agents/investigator.md)
  ...
  anything mid-investigation. [`docblock-checker`](.claude/agents/docblock-checker.md)
  is the same shape, used by `docblock-audit` — see below.
  [`test-coverage-reviewer`](.claude/agents/test-coverage-reviewer.md) is
  also the same shape: read-only, given a diff/PR/branch, it works out the
  realistic edge cases and checks whether tests actually cover them. Used
  standalone for a quick "are the important paths tested?" check, and by
  the `bug-fix` skill's step 8 to confirm the fix's edge cases are covered
  before the fix counts as complete.
- **Global conventions** (`.claude/CLAUDE.md`): install at
...
`.claude/tools/night_coder/` is a **bonus, experimental side
...
(`.claude/tools/db_readonly/db_server.py`).
...
- **`.env` file** next to `db_server.py`
  (`.claude/tools/db_readonly/.env`, not committed to git) with the
...
Run once, from inside `.claude/tools/db_readonly/`:
```
...
`.claude/agents/investigator.md` is a plain Claude Code subagent
...
`.claude/agents/test-coverage-reviewer.md` is the same shape again, for a
different job: given a diff/PR/branch, it identifies realistic edge cases
in the changed code and checks whether tests actually cover them (reading
the test bodies, not just their names) — never proposing or writing the
missing tests itself, just reporting the gap.

- **Standalone use**: ask it directly for a quick "are the important paths
  tested?" check on a diff or PR.
- **Used by the `bug-fix` skill**: step 8 delegates to it to confirm the
  regression fix's realistic edge cases are covered before the fix counts
  as complete, keeping that analysis out of the main conversation.
...
- **`docblock-checker`** (`.claude/agents/docblock-checker.md`):
...
Output: `.claude/tools/docblock_checker/reports/docblock_report.json`
...
(`.claude/tools/night_coder/night_coder.py`)
...
- `~/.claude/CLAUDE.md` installed with the conventions from this repo's
  `.claude/CLAUDE.md` (branch-first, no force/push, no k8s, ...) —
...
Output: `.claude/tools/night_coder/reports/night_coder_output_<date>.md`
